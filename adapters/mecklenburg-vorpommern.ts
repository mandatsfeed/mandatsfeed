// Parldok-MV-Adapter — Drucksachen (Anträge, Kleine/Große Anfragen,
// Gesetzentwürfe, Beschlussempfehlungen) im Landtag Mecklenburg-Vorpommern.
//
// Anders als bei PADOKA Sachsen-Anhalt ist Parldok kein SPA mit URL-Filtern,
// sondern eine JS-getriebene Server-Render-App. Pagination per `pd.resultpage(N)`,
// Pagesize konfigurierbar via `pd.pagesize`. Hardlimit pro Suche: 1000 Items.
//
// Aufruf:
//   pnpm run fetch:mecklenburg-vorpommern
//   WP=8 pnpm run fetch:mecklenburg-vorpommern
//
// Idempotent + additiv.
...
function fetchAllRecords(): RawRecord[] {
  ...
  if (waitForResults() === 0) {
    console.error("[parldok-mv] keine Treffer im initial-load");
    return [];
  }
  setPageSize100();
  const maxhits = readStateNumber("maxhits");
  const maxpages = readStateNumber("maxpages");
  console.log(`[parldok-mv] maxhits=${maxhits}, maxpages=${maxpages}`);
...
function buildActivity(raw: RawRecord, parsed: ParsedMeta): Activity | null {
  ...
  const a: Activity = {
    ...
    fraktionen,
    document: {
      url: `https://www.dokumentation.landtag-mv.de/parldok/dokument/${raw.docid}`,
    },
  };
