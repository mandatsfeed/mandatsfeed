...
// Angepasste Kopie von sachsen-anhalt-reden.ts. Code-Pfad ist identisch,
// aber: die globale Reden-Suche unter ?type=generic4&docart=Plenarprotokoll
// liefert in Brandenburg aktuell 0 Treffer (anders als bei PADOKA). Solange
// das so bleibt, schreibt der Lauf 0 Reden — die Logik ist trotzdem bereits
// drin, damit der Adapter direkt scharf geht, sobald Brandenburg die
// Plenarprotokoll-Indexierung freischaltet oder wir auf per-MdL-speaker-
// URLs (mit den 88 Einträgen aus der Registry) umstellen.
//
// Aufruf:
//   pnpm run fetch-reden:brandenburg              → Default-Jahr
//   YEAR=2026 pnpm run fetch-reden:brandenburg    → bestimmtes Jahr
//
// Idempotent + additiv.
...
const LISTING_URL =
  ...
  `?type=generic4&action=link&db=lbb.lissh&docart=Plenarprotokoll` +
...
function fetchAllSpeeches(): RawSpeechRecord[] {
  ...
  if (n === 0) { console.error("[brandenburg-reden] keine Treffer im initial-load"); return []; }
  const total = readTotal();
  console.log(`[brandenburg-reden] Treffer insgesamt: ${total}`);
  if (!clickAlleAufEinerSeite()) {
    console.error("[brandenburg-reden] 'Alle auf einer Seite' nicht gefunden");
...
function buildActivity(speakerName: string, speakerFraktion: string, slug: string, entry: RegistryEntry, sp: RawSpeechRecord): Activity | null {
  ...
  const id = `starweb-bb-rede-${plprSlug}${pageSuffix}-${slug}`;
...
function main(): void {
  ...
  console.log(`[brandenburg-reden] ${speeches.length} Speeches (${YEAR}) · ${written} neu · ${skipped} schon vorhanden · ${noSpeaker} ohne Speaker-Kontext · ${unknownSpeaker} Speaker nicht in Registry`);
}
