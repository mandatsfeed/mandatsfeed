// STARWEB-Adapter Landtag Brandenburg.
// Brandenburg nutzt dieselbe STARWEB-Plattform wie PADOKA Sachsen-Anhalt,
// daher ist diese Datei eine angepasste Kopie von adapters/padoka.ts.
// Unterschiede: type=generic1 statt generic2, db=lbb.lissh, eigene
// Fraktionen (inkl. BSW), ID-Prefix starweb-bb-.
//
// Aufruf:
//   pnpm run fetch:brandenburg               → Default-Jahr = aktuelles
//   YEAR=2026 pnpm run fetch:brandenburg     → bestimmtes Jahr
//
...
const LISTING_URL =
  ...
  `?type=generic1&action=link&db=lbb.lissh` +
...
function fetchAllRecords(): RawRecord[] {
  ...
  if (n === 0) {
    console.error("[starweb-bb] keine Treffer im initial-load");
    return [];
  }
  const { total } = readRangeFromTreffer();
  console.log(`[starweb-bb] Treffer insgesamt: ${total}`);
  if (!clickAlleAufEinerSeite()) {
    console.error("[starweb-bb] 'Alle auf einer Seite' Button nicht gefunden");
...
function parseMeta(meta: string): ParsedMeta | null {
  ...
  return {
    ...
    urheber,
    primaryDocLabel,
    relatedTo: relatedKind && relatedNr ? `starweb-bb-${relatedKind.toLowerCase()}-${relatedNr.replace("/", "-")}` : undefined,
  };
}
...
function buildActivity(raw: RawRecord, parsed: ParsedMeta): Activity | null {
  ...
  const activity: Activity = {
    id: `starweb-bb-${idPrefix}-${drsSlug}`,
    source: "starweb",
  ...
  if (raw.documentUrl) {
    activity.document = {
      url: raw.documentUrl.startsWith("http")
        ? raw.documentUrl
        : `https://www.parlamentsdokumentation.brandenburg.de${raw.documentUrl}`,
...
function main(): void {
  ...
  console.log(
    `[starweb-bb] ${records.length} Records (${YEAR}) · ${written} neu · ${skipped} schon vorhanden · ${nonMandate} keine Mandatsträger-Aktivität · ${unparsed.length} ungeparst`,
  );
